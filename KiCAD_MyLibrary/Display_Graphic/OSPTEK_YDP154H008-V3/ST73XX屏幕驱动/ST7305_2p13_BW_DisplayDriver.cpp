...
ST7305_2p13_BW_DisplayDriver::ST7305_2p13_BW_DisplayDriver(int dcPin, int resPin, int csPin, int sclkPin, int sdinPin, SPIClass& spi) : 
    DC_PIN(dcPin), RES_PIN(resPin), CS_PIN(csPin), SCLK_PIN(sclkPin), SDIN_PIN(sdinPin),
    LCD_WIDTH(122)/*132*/ , LCD_HIGH(250),
    ST73XX_UI(122, 250),
    // 122/4=30.5≈33(因为3byte写一次) 一行共33个byte的数据，上下两行共用一行的数据，所以总行数需要除2
    // 250/2=125 所以共125行，一行33个byte数据，共125*33=4125byte
    LCD_DATA_WIDTH(33), LCD_DATA_HIGH(125), DISPLAY_BUFFER_LENGTH(4125),
    spiRef(spi)
{
...
ST7305_2p13_BW_DisplayDriver::~ST7305_2p13_BW_DisplayDriver() {
    delete[] display_buffer;
}

void ST7305_2p13_BW_DisplayDriver::initialize() {
...
void ST7305_2p13_BW_DisplayDriver::fill(uint8_t data) {
    memset(display_buffer, data, DISPLAY_BUFFER_LENGTH);
    Serial.printf("fill data = 0x%x\n", data);
}

void ST7305_2p13_BW_DisplayDriver::clearDisplay() {
    memset(display_buffer, 0x00, DISPLAY_BUFFER_LENGTH);
}

void ST7305_2p13_BW_DisplayDriver::writePoint(uint x, uint y, bool enabled) {
...
void ST7305_2p13_BW_DisplayDriver::writePoint(uint x, uint y, uint16_t data) {
    if(x>=LCD_WIDTH || y>=LCD_HIGH){
        return;
    }
    else{
...
void ST7305_2p13_BW_DisplayDriver::display() {
    address();
...
void ST7305_2p13_BW_DisplayDriver::Initial_ST7305() {
    digitalWrite(RES_PIN, HIGH);	
    delay(10);
    ...
    Write_Parameter(0X0E); //VGH 00:8V  04:10V  08:12V   0E:15V
    Write_Parameter(0X05); //VGL 00:-5V   04:-7V   0A:-10V   05:-7.5V
    ...
    Write_Parameter(0X3F); //252 line = 63 * 4
    ...
    Write_Register(0xC7); //ultra low power code
...
void ST7305_2p13_BW_DisplayDriver::Low_Power_Mode(){
    Write_Register(0x39); //LPM:Low Power Mode ON
}

void ST7305_2p13_BW_DisplayDriver::High_Power_Mode(){
    Write_Register(0x38); //HPM:high Power Mode ON
}

void ST7305_2p13_BW_DisplayDriver::display_on(bool enabled){
    if(enabled){
        Write_Register(0x29); //DISPLAY ON  
    }else{
        Write_Register(0x28); //DISPLAY OFF  
...
void ST7305_2p13_BW_DisplayDriver::display_Inversion(bool enabled){
    if(enabled){
        Write_Register(0x21); //Display Inversion On 
    }else{
        Write_Register(0x20); //Display Inversion Off 
...
void ST7305_2p13_BW_DisplayDriver::address() {
    //WRITE RAM 122x250
    Write_Register(0x2A); //Column Address Setting
    Write_Parameter(0X19);
    Write_Parameter(0X23);
...
void ST7305_2p13_BW_DisplayDriver::Write_Register(uint8_t idat) {
...
void ST7305_2p13_BW_DisplayDriver::Write_Parameter(uint8_t ddat) {
    digitalWrite(DC_PIN, HIGH);
    digitalWrite(CS_PIN, LOW);
    spiRef.write(ddat);
    digitalWrite(CS_PIN, HIGH);
