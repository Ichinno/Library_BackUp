...
void loop() {
    ...
    display.drawLine(30, 20, 60, 70, ST7305_COLOR_BLACK);

    // 画一个实心三角形
    display.drawFilledTriangle(30, 5, 50, 35, 70, 5, ST7305_COLOR_BLACK);

    // 画一个实心三角形
    display.drawFilledTriangle(70, 5, 90, 35, 110, 5, ST7305_COLOR_BLACK);

    // 画一个实心三角形
    display.drawFilledTriangle(110, 5, 130, 35, 150, 5, ST7305_COLOR_BLACK);

    // 画一个三角形
    display.drawTriangle(150, 5, 170, 35, 190, 5, ST7305_COLOR_BLACK);

    // 画一个矩形
    display.drawRectangle(80, 40, 140, 70, ST7305_COLOR_BLACK);
    ...
    u8g2_for_st73xx.setForegroundColor(ST7305_COLOR_BLACK);      // apply Adafruit GFX color
    u8g2_for_st73xx.setFont(u8g2_font_inb24_mf);  // select u8g2 font from here: https://github.com/olikraus/u8g2/wiki/fntlistall
    u8g2_for_st73xx.setFontMode(1);                 // use u8g2 transparent mode (this is default)
    u8g2_for_st73xx.setCursor(0,352);                // start writing at this position
    ahtValue = aht20.readTemperature(); //read 6-bytes via I2C, takes 80 milliseconds
    u8g2_for_st73xx.printf("T:%02.2fC", ahtValue);

    u8g2_for_st73xx.setForegroundColor(ST7305_COLOR_BLACK);      // apply Adafruit GFX color
    u8g2_for_st73xx.setCursor(0,382);                // start writing at this position
    ahtValue = aht20.readHumidity(); //read another 6-bytes via I2C, takes 80 milliseconds
    u8g2_for_st73xx.printf("H:%02.2f%%", ahtValue);
