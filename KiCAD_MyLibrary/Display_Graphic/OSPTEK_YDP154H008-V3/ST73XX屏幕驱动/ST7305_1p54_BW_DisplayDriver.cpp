...
ST7305_1p54_BW_DisplayDriver::ST7305_1p54_BW_DisplayDriver(int dcPin, int resPin, int csPin, int sclkPin, int sdinPin, SPIClass& spi) : 
    DC_PIN(dcPin), RES_PIN(resPin), CS_PIN(csPin), SCLK_PIN(sclkPin), SDIN_PIN(sdinPin),
    LCD_WIDTH(200)/*132*/ , LCD_HIGH(200),
    ST73XX_UI(200, 200),
    // 200/4=50≈51(因为3byte写一次) 一行共51个byte的数据，上下两行共用一行的数据，所以总行数需要除2
    // 200/2=100 所以共100行，一行51个byte数据，共100*51=5100byte
    LCD_DATA_WIDTH(51), LCD_DATA_HIGH(100), DISPLAY_BUFFER_LENGTH(5100),
    spiRef(spi)
{
...
ST7305_1p54_BW_DisplayDriver::~ST7305_1p54_BW_DisplayDriver() {
    delete[] display_buffer;
}

void ST7305_1p54_BW_DisplayDriver::initialize() {
...
void ST7305_1p54_BW_DisplayDriver::fill(uint8_t data) {
    memset(display_buffer, data, DISPLAY_BUFFER_LENGTH);
    Serial.printf("fill data = 0x%x\n", data);
}

void ST7305_1p54_BW_DisplayDriver::clearDisplay() {
    memset(display_buffer, 0x00, DISPLAY_BUFFER_LENGTH);
}

void ST7305_1p54_BW_DisplayDriver::writePoint(uint x, uint y, bool enabled) {
...
void ST7305_1p54_BW_DisplayDriver::writePoint(uint x, uint y, uint16_t data) {
    if(x>=LCD_WIDTH || y>=LCD_HIGH){
        return;
    }
    else{
...
void ST7305_1p54_BW_DisplayDriver::display() {
    address();
...
void ST7305_1p54_BW_DisplayDriver::Initial_ST7305() {
    digitalWrite(RES_PIN, HIGH);	
    delay(10);
    ...
    Write_Parameter(0X32); //200 line = 50 * 4
...
void ST7305_1p54_BW_DisplayDriver::Low_Power_Mode(){
    Write_Register(0x39); //LPM:Low Power Mode ON
}

void ST7305_1p54_BW_DisplayDriver::High_Power_Mode(){
    Write_Register(0x38); //HPM:high Power Mode ON
}

void ST7305_1p54_BW_DisplayDriver::display_on(bool enabled){
    if(enabled){
        Write_Register(0x29); //DISPLAY ON  
    }else{
        Write_Register(0x28); //DISPLAY OFF  
...
void ST7305_1p54_BW_DisplayDriver::display_Inversion(bool enabled){
    if(enabled){
        Write_Register(0x21); //Display Inversion On 
    }else{
        Write_Register(0x20); //Display Inversion Off 
...
void ST7305_1p54_BW_DisplayDriver::address() {
    //WRITE RAM 200*200
    Write_Register(0x2A); //Column Address Setting
    Write_Parameter(0X16);
    Write_Parameter(0X26);
...
void ST7305_1p54_BW_DisplayDriver::Write_Register(uint8_t idat) {
...
void ST7305_1p54_BW_DisplayDriver::Write_Parameter(uint8_t ddat) {
    digitalWrite(DC_PIN, HIGH);
    digitalWrite(CS_PIN, LOW);
    spiRef.write(ddat);
    digitalWrite(CS_PIN, HIGH);
